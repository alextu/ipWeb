...
public class MyLogin extends CRIWebComponent {
    ...
    public MyLogin(WOContext context) {
		...
		domain = ((Application)criApp).domaineEtudiant();
    }
...
*/
    ...
    public static  boolean action(CRIWebSession session,EOEditingContext ec){
    	return (true);
    }
    ...
    public WOComponent goCas() {
    	...
    	page.setUrl(DirectAction.getLoginActionURL(this.context(),true,false));
        return page;
    }
...
//  	boolean identificationEtudiant = true;	
    	...
    	if(passOk)
    	{
    		...
    		if (user.errorCode() > 0) {
    			...
    			action(criSession(),localEContext());
...
//  				((Session)session()).setEmailCourant(email+"@"+domain);
    ...
    public boolean useMail() {
        LRLog.trace("useMail = "+((Application)application()).useEmailForLogin());
        return ((Application)application()).useEmailForLogin();
    }
    
    public boolean useIpAnonymes() {
        LRLog.trace("useIpAnonymes = "+((Application)application()).useIpAnonymes());
        return ((Application)application()).useIpAnonymes();
    }
    


    class Responder implements CRILoginResponder{

		public WOComponent loginAccepted(CRILogin arg0) {
			LRUserInfoDB user = new LRUserInfoDB(criApp.dataBus());
			user.compteForLogin(arg0.login,null,true);
//			LRLog.trace("pwd = "+arg0.password);
			if(user.errorCode()!=LRUserInfo.ERROR_NONE)
			{
				return ((Application)application()).pageErreur("Erreur Individu","Il y a eut un problème avec l'identification !",user.errorMessage(),null,context());
			}
			criSession().setConnectedUserInfo(user);
			action(criSession(),localEContext());
			Main page =  (Main)pageWithName("Main");
